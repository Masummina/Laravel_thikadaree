                       ...
                       @foreach($payment_list as $row)
                         ...
                         //  echo ($interval);
                           ...
                           @endphp
                                ...
                                <td>
                                    @if($row->status==0)
                                        <form  class="pull-right delete" action="{{ url('/payment/'.$row->id) }}" method="post" onsubmit="return confirm('Are you sure want to confirm approve?')" >
                                            {{method_field('DELETE')}}
                                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                            <button type="submit" class="btn btn-default">Pending</button>
                                        </form>
                                    @else
                                         Approve
                                    @endif
