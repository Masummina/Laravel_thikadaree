...
@section('content')
        ...
        <div class="col-md-12">
          <div class="box box-solid">
            ...
            // Set standard down payment & standard balance of down payment  
              ...
              $prospect_info = DB::table('customers')->where('id', $schedules_info->prospect_id)->first();
            ...
            <div class="col-md-6">
                <table class="table table-striped table-hover">
                    <tbody>
                    <tr> <td> Customer Name </td> <td>: {!! $prospect_info->prefix.' '.$prospect_info->name !!} </td> </tr>
                    <tr> <td> Handover date </td> <td>: {!! date("j F, Y",strtotime($project_info->hand_over)) !!} </td> </tr>
                    <tr> 
                        <td> Rebate  : {!! $schedules_info->redate_rate !!}% </td>  
                        <td> Delay Charge : {!! $schedules_info->delay_rate !!}% </td> 
                    </tr>
                ...
                <div class="col-md-6">
                    ...
                    <a href="{!! url('edit-schedules/'.$schedules_info->id) !!}" class="btn btn-success pull-left"> Edit Schedule </a>
                </div>
            </div>
            <!-- /.box-body -->
          </div>
            ...
            <!-- Second Time Down payment Check -->      
                    ...
                    @while($_DP_Balance>0)
                        @php 
                            ...
                            if($advance_deposited>0)
                            {                                    
                                if($advance_deposited >$_DP_Balance){
                                    $deposit_amount = $_DP_Balance;
                                    $advance_deposited = $advance_deposited - $_DP_Balance;                                     
                                    $_DP_Balance = 0;                                    
                                } else {                    
                                    $deposit_amount = $advance_deposited;
                                    $advance_deposited = 0;                    
    ...
    <!-----------************** Banalce Down Payment Start End ******************------------>                
        ...
        @php
            ...
            $SS_info_details = DB::table('pre_ss_details')
                    ->where('pre_schedule_id', $schedule_id)
                    ->orderBy('payment_date', 'asc')
                    ->get();
                    
            if(!isset($SS_info_details[0]))
            { 
                $SS_info_details = array();
                while($date_end>$date_start)
                {
                    $SS_info_details[] = array('payment_date'=>$date_start,'amount'=>$per_month_installment);
        ...
        @endphp
            ...
            @if($no_of_month>0)
                @foreach ($SS_info_details as $item)                
                    @php
                        if(isset($item->payment_date)){
                            $item = (array) $item;
                        }                     
                        $date_start = $item['payment_date'];
                        $per_month_installment = $item['amount'];
