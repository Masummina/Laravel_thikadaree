      ...
      <form onsubmit="return confirm('Are you sure you want to submit?')" action="{{ url('/bem-posts/'.$post_edit->id) }}" method="POST" enctype="multipart/form-data">
                       {{method_field('PUT')}}
                            ...
                            <div class="form-group">
                                ...
                                <select name="post_type"  class="form-control">
                                     <option value="0"> Select </option>
                                        ...
                                        <option value="slider" @if($post_edit->post_type=='slider') selected @endif >slider</option>
                                        <option value="tender" @if($post_edit->post_type=='tender') selected @endif >Tender</option>
                                        <option value="client" @if($post_edit->post_type=='client') selected @endif >client</option>
                                        <option value="service" @if($post_edit->post_type=='service') selected @endif >service</option>
                                        <option value="footer_left" @if($post_edit->post_type=='footer_left') selected @endif > Footer left</option>
                                        <option value="footer_middle" @if($post_edit->post_type=='footer_middle') selected @endif >Footer middle</option>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="discription" rows="5" id="discription">{!! $post_edit->discription !!}</textarea>
                            </div>

                            <div class="form-group">
                                <label for="short_discription">Short Discription:</label>
                                <textarea class="form-control" name="short_discription" rows="5" id="discription">{!! $post_edit->short_desc !!}</textarea>
                            </div>
