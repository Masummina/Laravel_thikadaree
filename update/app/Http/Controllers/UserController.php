...
class UserController extends Controller
{
	...
	public function PostProject(Request $request)
    {
		...
		if(isset($request->title))
		{
			...
			$min_max_budget = explode('to', $budgetRequest[1]); 
			//dd($min_max_budget);
			...
			'min_budget' => $min_max_budget[0],
			'max_budget' => $min_max_budget[1],
    ...
    public function profile(Request $request)
    {
		...
		// experience and education add query start
			if(isset($request->exp_title)){
				...
				if ($request->hasFile('certificate')) {
					...
					$name = 'EX_Certi'.time().'.'.$image->getClientOriginalExtension();
				...
				return Redirect::to('profile')->with('message', 'Experience  added  successfully!');
			};
			...
			if(isset($request->edu_country)){
				...
				return Redirect::to('profile')->with('message', 'Education added successfuly');
					...
					if(isset($request->certificate_title)){
						...
						if($validator->fails()){
							return Redirect::to('profile')->with('message', 'Certificate added  Fail');
						}

						if ($request->hasFile('certificate_image')) {
							$image = $request->file('certificate_image');
							$name = 'CER'.time().'.'.$image->getClientOriginalExtension();
							$destinationPath = public_path('/images/certificate/other');
							$image->move($destinationPath, $name);
							//dd($name);
						}else{
							$name = '';
						...
						$insert_cerfificate = DB::table('userprofile')->insert($data);
						if($insert_cerfificate){
							return Redirect::to('profile')->with('message', 'Certificate added successfuly');
						}else{
							return Redirect::to('profile')->with('message', 'Certificate added fails');
						}
		...
		// show certificate query 
		$certificate = DB::table('userprofile')->where('user_id', $useridd)->where('type', 'certificate')->orderBy('id', 'DESC')->get();
		//dd($certificate);
        return view('customer.profile', compact('userPro', 'experience', 'education', 'certificate'));
    }
